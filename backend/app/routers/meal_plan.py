...
def _meal_plan_to_out(plan: models.MealPlan) -> schemas.MealPlanOut:
    ...
    days = [schemas.MealPlanDayOut(date=d["date"], meal=schemas.MealPlanMealOut(**d["meal"])) for d in days_data]
...
def replace_day(
    ...
    """Replace the meal at day_index with a new AI-generated meal. Consumes one transformation quota."""
    if not current_user.is_verified:
    ...
    days_list[payload.day_index]["meal"] = new_meal
